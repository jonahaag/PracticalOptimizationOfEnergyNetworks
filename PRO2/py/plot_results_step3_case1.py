...
color_new_bio_oil = ["#9e9e9e"]
label_new_bio_oil = ["New Bio Oil"]
...
def plot_load_duration_stackplot(df, plots_folder, save):
    ...
    y5 = "Bio Oil Load Output A1 + A2 [MW]"
    y6 = "HOB_Load_Output_A3[Dim 1][MW]"
    fig, ax = plt.subplots()
    plt.stackplot(df_load_sorted["percentage"], 
                ...
                labels=labels[0:5]+label_new_bio_oil,
                colors=color_map[0:5]+color_new_bio_oil)
    plt.ylim(0, None)
    plt.xlim(0, None)
...
def plot_fuel_input(df, ax, title):
    ...
    y5 = "Bio Oil Fuel Input A1 + A2 [MW]"
    y6 = "HOB_Fuel_Input_A3[Dim 1][MW]"
...
def plot_fuel_cost(df, ax, electricity_price, title):
    ...
    ax.stackplot(df["Date (CEST)"], 
                ...
                df[y6].multiply(df["Bio Oil Price"]),
                labels=labels[0:5]+label_new_bio_oil,
                colors=color_map[0:5]+color_new_bio_oil)
    ax.xaxis.set_major_locator(ticker.MultipleLocator(2))
    ax.xaxis.set_major_formatter(ticker.ScalarFormatter())
...
def plot_subplots(df, electricity_price, plots_folder, save):
    ...
    for i, day_start_id in enumerate([1392, 4727, 7968, 2255]):
        ...
        figs[i].legend(lines[0:7], labels[0:5]+label_new_bio_oil+["Demand"], ncol=7, loc='center')
        figs[i].tight_layout()
        if save:
    ...
    for i, result_type in enumerate(["fuel_input", "dh_by_fuel", "electricity_production", "fuel_cost"]):
        ...
        lines, labels = [sum(lol, []) for lol in zip(*lines_labels)]
        if i == 1:
            figs[i+4].legend(lines[0:7], ["Demand"]+labels[1:6]+label_new_bio_oil, ncol=7, loc='center')
        elif i == 2:
            figs[i+4].legend(lines[0:3], labels[0:3], ncol=3, loc='center')
        else:
            figs[i+4].legend(lines[0:6], labels[0:5]+label_new_bio_oil, ncol=6, loc='center')
...
def plot_cost_revenue(df, electricity_price, plots_folder, save):
    ...
    oil_cost = df["Bio Oil Fuel Input A1 + A2 [MW]"].multiply(df["Bio Oil Price"]).sum()
    new_oil_cost = df["HOB_Fuel_Input_A3[Dim 1][MW]"].multiply(df["Bio Oil Price"]).sum()
    ...
    costs = sorted(list(zip([waste_cost, wood_cost, pellet_cost, electricity_cost, oil_cost, new_oil_cost],range(6))),reverse=True)
    for cost, i in costs:
        ax.bar("Cost", cost, width=width, bottom=bottom, label=label_new_bio_oil[0] if i==5 else labels[i], color=color_new_bio_oil[0] if i==5 else color_map[i])
        bottom += cost
...
def create_plots(df, electricity_price, save, show, case=0):
